  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        trailing: Text('$_steps', style: const TextStyle(fontSize: 20)),
      ),
    );
  }
}
...
double calculateBMR({
  ...
  if (isMale) {
    return 66 + (13.7 * weight) + (5 * heightCm) - (6.8 * age);
  } else {
...
double estimateCaloriesBurned(int steps,
    {double strideLength = 0.78, double caloriesPerKm = 50}) {
  double distance = (steps * strideLength) / 1000;
  return distance * caloriesPerKm;
}

double calculateSleepEfficiency(double hoursSlept, double timeInBed) {
  return (hoursSlept / timeInBed) * 100;
}

// ------------------ NEW FEATURE FUNCTIONS ------------------

// 1. Double Support Time (%)
double calculateDoubleSupportTime(double timeWithBothFeet, double totalGaitCycleTime) {
  if (totalGaitCycleTime == 0) return 0;
  return (timeWithBothFeet / totalGaitCycleTime) * 100;
}

// 2. Resting Energy (Basal Metabolic Rate - BMR) using the Harris–Benedict equations.
double calculateBMRHarrisBenedict({
  ...
  double heightCm = height * 100;
  if (isMale) {
    return 66.5 + (13.75 * weight) + (5.003 * heightCm) - (6.775 * age);
  } else {
...
// 3. Walking Asymmetry (%)
double calculateWalkingAsymmetry(double leftStepLength, double rightStepLength) {
  double sum = leftStepLength + rightStepLength;
  if (sum == 0) return 0;
  double avg = sum / 2;
  return ((leftStepLength - rightStepLength).abs() / avg) * 100;
}

// 4a. Walking Speed - Direct Method
double calculateWalkingSpeedDirect(double distance, double time) {
  if (time == 0) return 0;
  return distance / time;
}

// 4b. Walking Speed - Using Step Data
double calculateWalkingSpeedUsingStepData(double stepLength, double cadence) {
  // cadence in steps per second.
  return stepLength * cadence;
}

// 5. Walking Steadiness (%)
// (Standard Deviation of Step Time / Average Step Time) * 100
double calculateWalkingSteadiness(double stdStepTime, double avgStepTime) {
  if (avgStepTime == 0) return 0;
  return (stdStepTime / avgStepTime) * 100;
}

// 6. Walking Step Length (m)
double calculateStepLength(double totalDistanceWalked, int numberOfSteps) {
  if (numberOfSteps == 0) return 0;
  return totalDistanceWalked / numberOfSteps;
}

// 7. Activity (Active Energy Expenditure in kcal)
double calculateActiveEnergy(double metValue, double weight, double durationHours) {
  return (metValue - 1) * weight * durationHours;
}

// ------------------ EXTRA FEATURE FUNCTIONS & WIDGETS ------------------

// Function to share a health summary.
void shareHealthSummary() {
  final summary = "Health Summary:\n"
      "Steps: 11000\n"
      "Heart Rate: 72 bpm\n"
      "Calories: 500 kcal\n"
      "Water: 2L\n"
      "BMI: ${calculateBMI(70, 1.75).toStringAsFixed(1)}";
  Share.share(summary);
}

// A widget to show a random daily health tip.
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Text(tip, style: const TextStyle(fontSize: 16, fontFamily: 'Georgia')),
      ),
    );
  }
}

// A widget to display various health metrics.
class HealthMetrics extends StatelessWidget {
  const HealthMetrics({super.key});

  @override
  Widget build(BuildContext context) {
    // For demonstration, using fixed values.
    ...
    double hoursSlept = 7;
    double timeInBed = 8;
    // Sample values for new feature calculations:
    double timeWithBothFeet = 0.4;   // seconds
    double totalGaitCycleTime = 1.2;   // seconds
    double leftStepLength = 0.75;      // meters
    double rightStepLength = 0.70;     // meters
    double distance = 1000;            // meters
    double timeForWalk = 800;          // seconds
    double stepLength = calculateStepLength(1000, steps); // or a fixed value
    double cadence = 1.2;              // steps per second
    double stdStepTime = 0.1;          // seconds
    double avgStepTime = 0.8;          // seconds
    double metValue = 3.5;             // example MET value
    double durationHours = 1;          // 1 hour

    double bmi = calculateBMI(weight, height);
    double bmr = calculateBMR(weight: weight, height: height, age: age, isMale: isMale);
    double caloriesBurned = estimateCaloriesBurned(steps);
    double sleepEfficiency = calculateSleepEfficiency(hoursSlept, timeInBed);

    // New feature calculations:
    double doubleSupportTime = calculateDoubleSupportTime(timeWithBothFeet, totalGaitCycleTime);
    double bmrHarris = calculateBMRHarrisBenedict(weight: weight, height: height, age: age, isMale: isMale);
    double walkingAsymmetry = calculateWalkingAsymmetry(leftStepLength, rightStepLength);
    double walkingSpeedDirect = calculateWalkingSpeedDirect(distance, timeForWalk);
    double walkingSpeedStepData = calculateWalkingSpeedUsingStepData(stepLength, cadence);
    double walkingSteadiness = calculateWalkingSteadiness(stdStepTime, avgStepTime);
    double activeEnergy = calculateActiveEnergy(metValue, weight, durationHours);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text("BMI: ${bmi.toStringAsFixed(1)}", style: const TextStyle(fontFamily: 'Georgia')),
            Text("BMR (old): ${bmr.toStringAsFixed(0)} kcal/day", style: const TextStyle(fontFamily: 'Georgia')),
            Text("BMR (Harris–Benedict): ${bmrHarris.toStringAsFixed(0)} kcal/day", style: const TextStyle(fontFamily: 'Georgia')),
            Text("Estimated Calories Burned: ${caloriesBurned.toStringAsFixed(0)} kcal", style: const TextStyle(fontFamily: 'Georgia')),
            Text("Sleep Efficiency: ${sleepEfficiency.toStringAsFixed(0)}%", style: const TextStyle(fontFamily: 'Georgia')),
            const SizedBox(height: 10),
            // New feature outputs:
            Text("Double Support Time: ${doubleSupportTime.toStringAsFixed(1)}%", style: const TextStyle(fontFamily: 'Georgia')),
            Text("Walking Asymmetry: ${walkingAsymmetry.toStringAsFixed(1)}%", style: const TextStyle(fontFamily: 'Georgia')),
            Text("Walking Speed (Direct): ${walkingSpeedDirect.toStringAsFixed(2)} m/s", style: const TextStyle(fontFamily: 'Georgia')),
            Text("Walking Speed (Step Data): ${walkingSpeedStepData.toStringAsFixed(2)} m/s", style: const TextStyle(fontFamily: 'Georgia')),
            Text("Walking Steadiness: ${walkingSteadiness.toStringAsFixed(1)}%", style: const TextStyle(fontFamily: 'Georgia')),
            Text("Walking Step Length: ${stepLength.toStringAsFixed(2)} m", style: const TextStyle(fontFamily: 'Georgia')),
            Text("Active Energy Expenditure: ${activeEnergy.toStringAsFixed(0)} kcal", style: const TextStyle(fontFamily: 'Georgia')),
          ],
        ),
      ),
    );
...
// A reorderable dashboard to allow users to rearrange extra widgets.
class ReorderableDashboard extends StatefulWidget {
  const ReorderableDashboard({super.key});

  @override
  _ReorderableDashboardState createState() => _ReorderableDashboardState();
}
...
class _ReorderableDashboardState extends State<ReorderableDashboard> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Customize Dashboard", style: TextStyle(fontFamily: 'Georgia')),
      ),
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // Updated pages: Profile page removed. Profile now available via Settings.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          // Dashboard summary with live pedometer and other animated items.
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: [
              const StepCounter(),  // ← Live step count widget
              AnimatedSummaryItem(
              ...
              ElevatedButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const ReorderableDashboard(),
                    ),
                  );
                },
